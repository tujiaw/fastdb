...
dbExprNode* dbCompiler::rectangleConstant(dbExprNode* head)
{
    ...
    for (int i = 0; i < RECTANGLE_DIMENSION*2; i++) {
        if (head == NULL) {
            if (i == RECTANGLE_DIMENSION) {
                do {
                    r.boundary[i] = r.boundary[i-RECTANGLE_DIMENSION];
                } while (++i < RECTANGLE_DIMENSION*2);
                break;
            } else {
                error("Bad rectangle constant");
            }
        } else if (head->operand[0] == NULL) {
            error("Bad rectangle constant");
        }
...
dbExprNode* dbCompiler::term()
{
    ...
    switch (cop) {
      ...
      case tkn_distance:
      {
         ...
         if (r1->type == tpList) {
             r1 = rectangleConstant(r1);
         } else if (r1->type != tpRectangle) {
             error("DISTANCE function can be applied only to rectangles", pos);
         }
         dbExprNode* r2 = term();
         if (lex != tkn_rpar) {
             error("')' expected");
         ...
         lex = scan();
         if (r2->type == tpList) {
             r2 = rectangleConstant(r2);
         } else if (r2->type != tpRectangle) {
             error("DISTANCE function can be applied only to rectangles", pos);
         }
         return field(new dbExprNode(dbvmRectangleDistance, r1, r2), NULL, NULL);
      }
      case tkn_abs:
      case tkn_area:
      case tkn_length:
